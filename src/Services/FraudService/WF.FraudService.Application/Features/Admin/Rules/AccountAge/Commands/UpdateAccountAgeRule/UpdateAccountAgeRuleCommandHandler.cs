using MediatR;
using WF.FraudService.Domain.Abstractions;
using WF.FraudService.Domain.Entities;
using WF.Shared.Contracts.Abstractions;
using WF.Shared.Contracts.Result;

namespace WF.FraudService.Application.Features.Admin.Rules.AccountAge.Commands.UpdateAccountAgeRule;

public class UpdateAccountAgeRuleCommandHandler(
    IAccountAgeRuleRepository _repository,
    IUnitOfWork _unitOfWork) : IRequestHandler<UpdateAccountAgeRuleCommand, Result>
{
    public async Task<Result> Handle(UpdateAccountAgeRuleCommand request, CancellationToken cancellationToken)
    {
        var rule = await _repository.GetByIdAsync(request.Id, cancellationToken);
        if (rule is null)
        {
            return Result.Failure(Error.NotFound("AccountAgeRule.NotFound", $"Account age rule with ID {request.Id} not found."));
        }

        if (request.MinAccountAgeDays.HasValue)
        {
            var updateResult = rule.UpdateMinAccountAgeDays(request.MinAccountAgeDays.Value);
            if (updateResult.IsFailure)
            {
                return updateResult;
            }
        }

        if (request.MaxAllowedAmount is not null)
        {
            rule.UpdateMaxAllowedAmount(request.MaxAllowedAmount);
        }

        if (request.Description is not null)
        {
            rule.UpdateDescription(request.Description);
        }

        if (request.IsActive)
        {
            rule.Activate();
        }
        else
        {
            rule.Deactivate();
        }

        await _repository.UpdateAsync(rule, cancellationToken);
        await _unitOfWork.SaveChangesAsync(cancellationToken);

        return Result.Success();
    }
}
